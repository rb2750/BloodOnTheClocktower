    ...
    <section className="mt-6 px-5">
      <div className="mb-2 flex items-baseline justify-between">
        ...
        <span className="caps text-(--text-faint)">
          {messages.length === 1 ? newest.at : `${messages.length} notes`}
        </span>
      </div>
      <HoldToReveal
        ...
        outline={
          <span className="flex items-center justify-center gap-3">
            {messages.slice(-5).map((m) => (
              <span key={m.id} className="reveal-seal reveal-seal-small" />
            ))}
            {messages.length > 5 && (
              <span className="caps text-(--text-faint)">+{messages.length - 5}</span>
            )}
          </span>
        }
      >
